...
export const GraphView: React.FC<GraphViewProps> = ({ notes, topics }) => {
  ...
  const graphData = useMemo(() => {
    ...
    notes.forEach((note) => {
      ...
      const topicId = topic ? `topic-${topic.id}` : null;

      nodes.push({
        id: `note-${String(note.id)}`,
        name: note.title === "Sem Tópico" ? note.content.substring(0, 20) + "..." : note.content.substring(0, 30) + "...",
        val: 3,
        color: color,
        isTopic: false,
      });
  ...
  const paintNode = useCallback(
    ...
    (node: any, ctx: CanvasRenderingContext2D, globalScale: number) => {
      ...
      const fontSize = node.isTopic ? 14 / globalScale : 10 / globalScale;
      ctx.font = `bold ${String(fontSize)}px "Inter", sans-serif`;

      ctx.beginPath();
      ctx.arc(node.x, node.y, node.val, 0, 2 * Math.PI, false);
      ctx.fillStyle = String(node.color);
      ctx.fill();

      // Glow effect for topics
      if (node.isTopic) {
        ctx.shadowBlur = 15;
        ctx.shadowColor = String(node.color);
      } else {
        ctx.shadowBlur = 0;
      }
      ...
      ctx.fillStyle = node.isTopic ? "#ffffff" : "rgba(255,255,255,0.7)";
